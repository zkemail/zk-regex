...
use regex_automata::{ nfa::thompson::{ State, pikevm::PikeVM }, util::primitives::StateID };
            ...
            match state {
                ...
                State::ByteRange { trans } => {
                    ...
                    for byte in trans.start..=trans.end {
                        graph.nodes[state_id_usize].byte_transitions
                            .entry(byte)
                ...
                State::Union { alternates } => {
                    ...
                    graph.nodes[state_id_usize].epsilon_transitions.extend(
                        alternates.iter().map(|id| id.as_usize())
                    );
                }
                State::BinaryUnion { alt1, alt2 } => {
                    // Add epsilon transitions for both alternates
                    graph.nodes[state_id_usize].epsilon_transitions.push(alt1.as_usize());
                    graph.nodes[state_id_usize].epsilon_transitions.push(alt2.as_usize());
                }
                State::Capture { next, group_index, slot, .. } => {
                    // Add epsilon transition for capture
                    graph.nodes[state_id_usize].epsilon_transitions.push(next.as_usize());

                    // Add capture group information
                    let is_start = slot.as_usize() % 2 == 0;
                    graph.nodes[state_id_usize].capture_groups.push((
