...
// regex: foo|bar|baz
template SimpleAlternationRegex(maxHaystackBytes, maxMatchBytes) {
    ...
    var numTransitions = 9;
    var startStates[numStartStates] = [0, 5];
    ...
    for (var i = 0; i < maxMatchBytes; i++) {
        ...
        // Transition 0: 0 -[98]-> 2
        isValidTransition[0][i] <== CheckByteTransition()(0, 2, 98, currStates[i], nextStates[i], haystack[i]);
        // Transition 1: 0 -[102]-> 4
        isValidTransition[1][i] <== CheckByteTransition()(0, 4, 102, currStates[i], nextStates[i], haystack[i]);
        // Transition 2: 1 -[114]-> 6
        isValidTransition[2][i] <== CheckByteTransition()(1, 6, 114, currStates[i], nextStates[i], haystack[i]);
        // Transition 3: 1 -[122]-> 6
        isValidTransition[3][i] <== CheckByteTransition()(1, 6, 122, currStates[i], nextStates[i], haystack[i]);
        // Transition 4: 2 -[97]-> 1
        isValidTransition[4][i] <== CheckByteTransition()(2, 1, 97, currStates[i], nextStates[i], haystack[i]);
        // Transition 5: 3 -[111]-> 6
        isValidTransition[5][i] <== CheckByteTransition()(3, 6, 111, currStates[i], nextStates[i], haystack[i]);
        // Transition 6: 4 -[111]-> 3
        isValidTransition[6][i] <== CheckByteTransition()(4, 3, 111, currStates[i], nextStates[i], haystack[i]);
        // Transition 7: 5 -[98]-> 2
        isValidTransition[7][i] <== CheckByteTransition()(5, 2, 98, currStates[i], nextStates[i], haystack[i]);
        // Transition 8: 5 -[102]-> 4
        isValidTransition[8][i] <== CheckByteTransition()(5, 4, 102, currStates[i], nextStates[i], haystack[i]);
