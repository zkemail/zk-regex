...
// regex: (?:a(?:b|c)d)+
template NestedNoncapturingRegex(maxHaystackBytes, maxMatchBytes) {
    ...
    for (var i = 0; i < maxMatchBytes; i++) {
        ...
        // Transition 0: 0 -[97]-> 2
        isValidTransition[0][i] <== CheckByteTransition()(0, 2, 97, currStates[i], nextStates[i], haystack[i]);
        // Transition 1: 1 -[97]-> 2
        isValidTransition[1][i] <== CheckByteTransition()(1, 2, 97, currStates[i], nextStates[i], haystack[i]);
        // Transition 2: 2 -[98-99]-> 3
        isValidTransition[2][i] <== CheckByteRangeTransition()(2, 3, 98, 99, currStates[i], nextStates[i], haystack[i]);
        // Transition 3: 3 -[100]-> 4
        isValidTransition[3][i] <== CheckByteTransition()(3, 4, 100, currStates[i], nextStates[i], haystack[i]);
        // Transition 4: 4 -[97]-> 2
        isValidTransition[4][i] <== CheckByteTransition()(4, 2, 97, currStates[i], nextStates[i], haystack[i]);
