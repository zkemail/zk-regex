...
// regex: (?:[a-z]+[0-9]+)+
template NestedQuantifiedGroupsRegex(maxHaystackBytes, maxMatchBytes) {
    ...
    for (var i = 0; i < maxMatchBytes; i++) {
        ...
        // Transition 2: 2 -[97-122]-> 2
        isValidTransition[2][i] <== CheckByteRangeTransition()(2, 2, 97, 122, currStates[i], nextStates[i], haystack[i]);
        // Transition 3: 2 -[48-57]-> 3
        isValidTransition[3][i] <== CheckByteRangeTransition()(2, 3, 48, 57, currStates[i], nextStates[i], haystack[i]);
        // Transition 4: 3 -[97-122]-> 2
        isValidTransition[4][i] <== CheckByteRangeTransition()(3, 2, 97, 122, currStates[i], nextStates[i], haystack[i]);
        // Transition 5: 3 -[48-57]-> 3
        isValidTransition[5][i] <== CheckByteRangeTransition()(3, 3, 48, 57, currStates[i], nextStates[i], haystack[i]);
