...
template Caret4Regex(msg_bytes) {
	...
	// substrings calculated: [{(0, 1), (0, 2), (1, 1), (2, 1)}]
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(0, 1), (0, 2), (1, 1), (2, 1)]
		is_substr0[i] <== MultiOR(4)([states[i+1][0] * states[i+2][1], states[i+1][0] * states[i+2][2], states[i+1][1] * states[i+2][1], states[i+1][2] * states[i+2][1]]);
