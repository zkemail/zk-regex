...
template InternationalCharsDecomposed(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[4][i].in[1] <== and[32][i].out;
		...
		and[35][i].b <== multi_or[7][i].out;
		and[36][i] = AND();
		and[36][i].a <== states[i][28];
		and[36][i].b <== and[22][i].out;
