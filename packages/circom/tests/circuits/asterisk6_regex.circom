...
// regex: .*
template Asterisk6Regex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		and[20][i] = AND();
		and[20][i].a <== 1;
		and[20][i].b <== eq[46][i].out;
		states_tmp[i+1][7] <== 0;
		from_zero_enabled[i] <== MultiNOR(7)([states_tmp[i+1][1], states_tmp[i+1][2], states_tmp[i+1][3], states_tmp[i+1][4], states_tmp[i+1][5], states_tmp[i+1][6], states_tmp[i+1][7]]);
		states[i+1][0] <== MultiOR(2)([states_tmp[i+1][0], from_zero_enabled[i] * and[1][i].out]);
		states[i+1][1] <== MultiOR(2)([states_tmp[i+1][1], from_zero_enabled[i] * and[5][i].out]);
		states[i+1][2] <== MultiOR(2)([states_tmp[i+1][2], from_zero_enabled[i] * and[11][i].out]);
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][0];
	}
	out <== final_state_result.out;
	signal is_consecutive[msg_bytes+1][3];
	is_consecutive[msg_bytes][2] <== 0;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][0] * (1 - is_consecutive[msg_bytes-i][2]) + is_consecutive[msg_bytes-i][2];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
		is_consecutive[msg_bytes-1-i][2] <== ORAnd()([(1 - from_zero_enabled[msg_bytes-i+1]), states[num_bytes-i][0], is_consecutive[msg_bytes-1-i][1]]);
	}
