...
describe("Simple Regex Decomposed", () => {
  let circuit;
  beforeAll(async () => {
    const email_addr_json = fs.readFileSync(path.join(__dirname, "./circuits/international_chars_decomposed.json"), "utf8")
    const circom = wasm.gen_from_decomposed_memory(email_addr_json, 'InternationalCharsDecomposed');
    fs.writeFileSync(path.join(__dirname, "./circuits/international_chars_decomposed.circom"), circom);
