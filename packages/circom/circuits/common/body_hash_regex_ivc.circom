...
template BodyHashRegexIVC(chunk_bytes) {
    var num_states = 35;
    assert(num_states <= 254);
    var num_reveals = 1;
    signal input ivc_input[5+num_reveals]; // (init_idx, init_rand_hash, init_params_hash, init_is_accepted, init_msg_rlc, init_reveal0s_rlc)
	signal input external_inputs[chunk_bytes + 1 + num_states + 1 + 1 + 1 + 1]; // (chunk_msg, init_rand, init_states, init_is_consecutive, last_is_consecutive, init_from_zero_enabled, last_from_zero_enabled)
	signal output ivc_output[5+num_reveals]; // (last_idx, last_rand_hash, last_params_hash, last_is_accepted, last_msg_rlc, last_reveal0s_rlc)

    signal init_idx <== ivc_input[0];
    signal init_rand_hash <== ivc_input[1];
    signal init_params_hash <== ivc_input[2];
    signal init_is_accepted <== ivc_input[3];
    signal init_msg_rlc <== ivc_input[4];
    signal init_reveal0s_rlc <== ivc_input[5];

    signal in[chunk_bytes];
    for (var i = 0; i < chunk_bytes; i++) {
        in[i] <== external_inputs[i];
    }
    signal init_rand <== external_inputs[chunk_bytes];
    signal init_states[num_states];
    for (var i = 0; i < num_states; i++) {
        init_states[i] <== external_inputs[chunk_bytes + 1 + i];
    }
    signal init_is_consecutive <== external_inputs[chunk_bytes + 1 + num_states];
    signal last_is_consecutive <== external_inputs[chunk_bytes + 1 + num_states + 1];
    signal init_from_zero_enabled <== external_inputs[chunk_bytes + 1 + num_states + 1 + 1];
    signal last_from_zero_enabled <== external_inputs[chunk_bytes + 1 + num_states + 1 + 1 + 1];
    
    signal init_rand_hash_computed <== Poseidon(1)([init_rand]);
    init_rand_hash === init_rand_hash_computed;
    signal init_states_packed <== Bits2Num(num_states)(init_states);
    signal init_params_hash_computed <== Poseidon(4)([init_rand, init_states_packed, init_is_consecutive, init_from_zero_enabled]);
    init_params_hash === init_params_hash_computed;

    signal is_start_zero <== IsZero()(init_idx);
    is_start_zero * (in[0] - 255) === 0;
    is_start_zero * (init_states_packed - 1) === 0;
    is_start_zero * init_is_consecutive === 0;
    is_start_zero * init_from_zero_enabled === 0;
	...
	component eq[90][chunk_bytes];
	component lt[24][chunk_bytes];
	component and[76][chunk_bytes];
	component multi_or[19][chunk_bytes];
	signal states[chunk_bytes+1][num_states];
	signal states_tmp[chunk_bytes][35];
	signal from_zero_enabled[chunk_bytes + 1];
    component state_changed[chunk_bytes];

    from_zero_enabled[chunk_bytes] <== last_from_zero_enabled;
    ...
    for (var i = 0; i < chunk_bytes; i++) {
		...
		and[75][i].b <== eq[75][i].out;
		states[i+1][34] <== and[75][i].out;
		from_zero_enabled[i] <== MultiNOR(34)([states_tmp[i][1], states_tmp[i][2], states[i+1][3], states[i+1][4], states[i+1][5], states[i+1][6], states[i+1][7], states[i+1][8], states[i+1][9], states[i+1][10], states[i+1][11], states[i+1][12], states[i+1][13], states[i+1][14], states[i+1][15], states[i+1][16], states[i+1][17], states[i+1][18], states[i+1][19], states[i+1][20], states[i+1][21], states[i+1][22], states[i+1][23], states[i+1][24], states[i+1][25], states[i+1][26], states[i+1][27], states[i+1][28], states[i+1][29], states[i+1][30], states[i+1][31], states[i+1][32], states[i+1][33], states[i+1][34]]);
		states[i+1][1] <== MultiOR(2)([states_tmp[i][1], from_zero_enabled[i] * and[0][i].out]);
		states[i+1][2] <== MultiOR(2)([states_tmp[i][2], from_zero_enabled[i] * and[1][i].out]);
		state_changed[i].in[0] <== states[i+1][1];
		state_changed[i].in[1] <== states[i+1][2];
		state_changed[i].in[2] <== states[i+1][3];
		state_changed[i].in[3] <== states[i+1][4];
    ...
    states[chunk_bytes][0] <== 0;
    init_from_zero_enabled === from_zero_enabled[0];
	...
	// }
    // out <== is_accepted.out;
    component is_accepted = MultiOR(chunk_bytes+2);
	for (var i = 0; i <= chunk_bytes; i++) {
		is_accepted.in[i] <== states[i][34];
	}
    is_accepted.in[chunk_bytes + 1] <== init_is_accepted;
    signal last_is_accepted <== is_accepted.out;
	...
	// }
    signal is_consecutive[chunk_bytes+1][3];
	is_consecutive[chunk_bytes][2] <== last_is_consecutive;
    for(var i = chunk_bytes - 1; i >= 0; i--) {
        is_consecutive[i][0] <== states[i+1][34] * (1 - is_consecutive[i+1][2]) + is_consecutive[i+1][2];
        is_consecutive[i][1] <== state_changed[i].out * is_consecutive[i][0];
        is_consecutive[i][2] <== ORAnd()([(1 - from_zero_enabled[i+1]), states[i+1][34], is_consecutive[i][1]]);
    }
    init_is_consecutive === is_consecutive[0][2];
	...
	// }
    signal prev_states0[2][chunk_bytes];
    signal is_substr0[chunk_bytes];
    signal is_reveal0[chunk_bytes];
    signal reveal0[chunk_bytes];
    for (var i = 0; i < chunk_bytes; i++) {
        // the 0-th substring transitions: [(32, 33), (33, 33)]
        prev_states0[0][i] <== (1 - from_zero_enabled[i]) * states[i][32];
        prev_states0[1][i] <== (1 - from_zero_enabled[i]) * states[i][33];
        is_substr0[i] <== MultiOR(2)([prev_states0[0][i] * states[i+1][33], prev_states0[1][i] * states[i+1][33]]);
        is_reveal0[i] <== MultiAND(3)([last_is_accepted, is_substr0[i], is_consecutive[i][2]]);
        reveal0[i] <== in[i] * is_reveal0[i];
    }

    signal last_idx <== init_idx + chunk_bytes;
    signal rands[chunk_bytes+1] <== GenRLCRands(chunk_bytes)(init_rand);
    signal last_rand <== rands[chunk_bytes];
    signal last_rand_hash <== Poseidon(1)([last_rand]);
    signal last_states[num_states];
    for (var i = 0; i < num_states; i++) {
        last_states[i] <== states[chunk_bytes][i];
    }
    signal last_states_packed <== Bits2Num(num_states)(last_states);
    signal last_params_hash <== Poseidon(4)([last_rand, last_states_packed, last_is_consecutive, last_from_zero_enabled]);
    signal last_msg_rlc <== RLC(chunk_bytes)(in, init_msg_rlc, rands);
    signal last_reveal0s_rlc <== RLC(chunk_bytes)(reveal0, init_reveal0s_rlc, rands);
    ivc_output[0] <== last_idx;
    ivc_output[1] <== last_rand_hash;
    ivc_output[2] <== last_params_hash;
    ivc_output[3] <== last_is_accepted;
    ivc_output[4] <== last_msg_rlc;
    ivc_output[5] <== last_reveal0s_rlc;
}

// component main {public [ivc_input]} = BodyHashRegexIVC(32);