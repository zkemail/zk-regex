...
template FromAllRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[1][i].in[8] <== eq[7][i].out;
		multi_or[1][i].in[9] <== eq[8][i].out;
		multi_or[1][i].in[10] <== eq[9][i].out;
		multi_or[1][i].in[11] <== eq[10][i].out;
		multi_or[1][i].in[12] <== eq[11][i].out;
